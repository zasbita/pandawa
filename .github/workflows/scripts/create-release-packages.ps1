...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)memory/', '.pandawa/memory/'
    $Content = $Content -replace '(/?)scripts/', '.pandawa/scripts/'
    $Content = $Content -replace '(/?)templates/', '.pandawa/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        if ([string]::IsNullOrEmpty($scriptCommand)) {
            # Only warn when the command actually references {SCRIPT}; a prompt-only
            # command (e.g. the /pandawa router) needs no script.
            if ($fileContent -match '\{SCRIPT\}') {
        ...
        # Generate output file based on extension. The root entrypoint stays "/pandawa"
        # (file "pandawa.<ext>"); every other command is namespaced "pandawa.<name>".
        if ($name -eq 'pandawa') {
            $outputFile = Join-Path $OutputDir "pandawa.$Extension"
        } else {
            $outputFile = Join-Path $OutputDir "pandawa.$name.$Extension"
        }
...
function Generate-CopilotPrompts {
    ...
    # Match both the namespaced commands (pandawa.<name>.agent.md) and the root
    # entrypoint (pandawa.agent.md) so /pandawa gets a companion prompt too.
    $agentFiles = Get-ChildItem -Path "$AgentsDir/pandawa*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".pandawa"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .pandawa"
    }
    ...
    if (Test-Path "templates") {
        ...
        Write-Host "Copied templates -> .pandawa/templates"
    }
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "pandawa-template-${Agent}-${Script}-${Version}.zip"
...
function Normalize-List {
    # NOTE: do not name this parameter $Input - that is an automatic PowerShell
    # variable (the pipeline enumerator), which silently breaks the filter.
    param([string]$Value)
...
Get-ChildItem -Path $GenReleasesDir -Filter "pandawa-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}