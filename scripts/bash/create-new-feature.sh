...
find_repo_root() {
    local dir="$1"
    while [ "$dir" != "/" ]; do
        if [ -d "$dir/.git" ] || [ -d "$dir/.pandawa" ]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    >&2 echo "[pandawa] Warning: Branch name exceeded GitHub's 244-byte limit"
    >&2 echo "[pandawa] Original: $ORIGINAL_BRANCH_NAME (${#ORIGINAL_BRANCH_NAME} bytes)"
    >&2 echo "[pandawa] Truncated to: $BRANCH_NAME (${#BRANCH_NAME} bytes)"
fi

if [ "$HAS_GIT" = true ]; then
    git checkout -b "$BRANCH_NAME"
else
    >&2 echo "[pandawa] Warning: Git repository not detected; skipped branch creation for $BRANCH_NAME"
fi

FEATURE_DIR="$SPECS_DIR/$BRANCH_NAME"
mkdir -p "$FEATURE_DIR"

TEMPLATE="$REPO_ROOT/.pandawa/templates/spec-template.md"
SPEC_FILE="$FEATURE_DIR/spec.md"
if [ -f "$TEMPLATE" ]; then cp "$TEMPLATE" "$SPEC_FILE"; else touch "$SPEC_FILE"; fi

# Set the PANDAWA_FEATURE environment variable for the current session
export PANDAWA_FEATURE="$BRANCH_NAME"
...
if $JSON_MODE; then
    ...
    echo "PANDAWA_FEATURE environment variable set to: $BRANCH_NAME"
fi
